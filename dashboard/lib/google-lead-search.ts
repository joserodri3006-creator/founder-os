...
type GoogleSearchItem = { title?: string; link?: string; displayLink?: string; snippet?: string };
type SerperSearchItem = { title?: string; link?: string; snippet?: string };
...
function normalizedResult(item: GoogleSearchItem): LeadSearchResult | null {
  ...
  if (!displayLink) {
    try { displayLink = new URL(website).hostname; } catch { displayLink = website; }
  }
  return {
    title,
...
function filterAlreadyImported(results: LeadSearchResult[], existing: { urls: Set<string>; hosts: Set<string> }) {
...
async function searchWithSerper(apiKey: string, query: string, limit: number) {
  ...
  if (!response.ok) {
    return { error: data.message || "Die Google-Suche über Serper konnte nicht ausgefuehrt werden.", status: response.status >= 500 ? 502 : 400 };
  }
  return {
    results: (data.organic ?? []).map((item) => normalizedResult(item)).filter((item): item is LeadSearchResult => Boolean(item)),
    provider: "serper" as const,
  };
}

async function searchWithGoogleCustom(apiKey: string, engineId: string, query: string, limit: number) {
  const params = new URLSearchParams({ key: apiKey, cx: engineId, q: query, num: String(limit), hl: "de", gl: "de", safe: "active" });
  const response = await fetch(`https://www.googleapis.com/customsearch/v1?${params}`, { headers: { Accept: "application/json" }, cache: "no-store" });
  ...
  if (!response.ok) {
    if (data.error?.message?.includes("does not have the access to Custom Search JSON API")) {
      return { error: "Google Custom Search ist für neue Projekte nicht mehr verfügbar. Bitte SERPER_API_KEY in Vercel hinterlegen und neu deployen.", status: 503 };
    }
    return { error: data.error?.message || "Die Google-Suche konnte nicht ausgefuehrt werden.", status: response.status >= 500 ? 502 : 400 };
  }
  return {
    results: (data.items ?? []).map((item) => normalizedResult(item)).filter((item): item is LeadSearchResult => Boolean(item)),
    provider: "google_custom_search" as const,
  };
}

export async function searchGoogleLeads(input: { region: string; segment?: string; specialization?: string; limit?: number }) {
  ...
  if (!region) return { error: "Bitte eine Region angeben.", status: 400 };
  if (!segmentTerm) return { error: "Ungueltige Zielgruppe.", status: 400 };
  ...
  if (!serperApiKey && (!googleApiKey || !googleEngineId)) {
    return { error: "Google Search ist noch nicht konfiguriert. Bitte SERPER_API_KEY serverseitig in Vercel hinterlegen.", status: 503 };
  }
